...
export default function FacturaPage() {
  ...
  // Solo RUC (06) y CE (04) para factura
  ...
  const totales = useMemo(() => {
    ...
    // Gratuitas: 11 (gravado gratuito), 21 (exonerado gratuito), 31 (inafecto gratuito)
    const gratuitas = detalles.filter(d => esGratuito(d.tipoAfectacionIGV ?? ''))
      .reduce((acc, d) => acc + (d.baseIgv ?? 0), 0)
  ...
  useEffect(() => {
    ...
    // Legends según tipo de operación
  ...
  // ── Filtrar clientes — solo RUC y CE ─────────────────────────
  const clientesFiltrados = clientes.filter(c => {
    // Solo mostrar clientes con RUC (06) o CE (04)
  ...
  const seleccionarProducto = (producto: ProductoSucursal, index: number) => {
    ...
    nuevos[index] = {
      ...
      porcentajeIGV,
      codigoTipoDescuento: '01',
      _incluirIGV: producto.incluirIGV, _precioBase: precioBase,  _precioBaseOriginal: precioBase,
  ...
  const actualizarPrecioVenta = (index: number, nuevoPrecioVenta: number) => {
    ...
    if (esGratuito) return // gratuitas no permiten editar precio venta
  ...
  const actualizarPorcentajeIGV = (index: number, porcentaje: number) => {
    ...
    const nuevaPrecioBase = (tipoAfectacion === '10' || tipoAfectacion === '11')
      ...
      : precioVentaConIGV
    const calc = calcularDetalle(nuevaPrecioBase, precioVentaConIGV, d.cantidad ?? 1, porcentaje, tipoAfectacion, d.codigoTipoDescuento ?? '01', d.descuentoUnitario ?? 0)
    const nuevos = [...detalles]
  ...
  setDetalles(prev => {
    ...
    // ✅ siempre usar el precio original del producto, nunca el modificado
  ...
  const emitirComprobante = async () => {
    ...
    if (totales.importeTotal > 700 && !aplicarDetraccion && !totales.soloGratuitas) {
    ...
    if (itemSinDescripcion !== -1) {
      showToast(`El ítem ${itemSinDescripcion + 1} no tiene descripción o producto`, 'error'); return
    }
    // Validar detracción si está activa
    if (aplicarDetraccion && !detraccion.cuentaBancoDetraccion) {
    ...
    try {
      ...
      const resFactura = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/api/Comprobantes/GenerarXml`, facturaFinal, { headers: { Authorization: `Bearer ${accessToken}` } })
      ...
      const itemsParaStock = detalles.filter(d =>
        d.productoId && d.productoId !== 0 && d._sucursalProductoId && d._tipoProducto === 'BIEN'
      )
  ...
  useEffect(() => { console.log('factura:', factura); }, [factura]);
                  ...
                  <div className="flex gap-2">
                    {user?.rol === 'superadmin' ? (
                      <SelectPersonalizado
                        ...
                        opciones={sucursales.map((s: Sucursal) => ({
                          value: s.sucursalId,
                          label: `${s.serieFactura} - ${s.nombre ?? s.codEstablecimiento}`,
                          labelCorto: s.serieFactura,
                        }))}
                        onChange={(id) => {
                          const seleccionada = sucursales.find((s: Sucursal) => s.sucursalId === id)
                          if (!seleccionada) return
                          setSucursal(seleccionada)
                          setCorrelativoActual(seleccionada.correlativoFactura)
                          setFactura(prev => ({
                            ...prev,
                            serie: seleccionada.serieFactura,
                            correlativo: String(seleccionada.correlativoFactura).padStart(8, '0')
                          }))
                        }}
                      />
                    ) : (
                      <select disabled className="w-1/3 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none px-2 text-sm">
                        <option>{loadingSucursal ? 'Cargando...' : sucursal?.serieFactura}</option>
                      </select>
                    )}
                    <input type="text" disabled
                      value={String(correlativoActual ?? sucursal?.correlativoFactura ?? '').padStart(8, '0')}
                    ...
                    onChange={(e) => {
                      ...
                      if (detalles.length > 0) {
                        setDetalles(prev => prev.map(d => {
                          ...
                          const porcentajeIGV = d.porcentajeIGV ?? 18
                          const tipoAfectacion = d.tipoAfectacionIGV ?? '10'
                          const nuevoPrecioVenta = esGratuito ? 0 : (tipoAfectacion === '10'
                          ...
                          <div className="space-y-1.5">
                            ...
                            <div className="flex gap-2">
                              <input type="number" value={pago.monto}
                                ...
                                // ✅ desactivar si es Efectivo y es el único medio de pago
              ...
              <div className="border border-gray-100 rounded-xl overflow-hidden">
                <button type="button" onClick={() => setShowDetraccion(!showDetraccion)}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {aplicarDetraccion && (
                      <span className="text-[10px] bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full font-medium">Activa</span>
                    )}
                  </div>
                              ...
                              <td className="px-2 py-1.5">
                                {!d.productoId ? (
                                  <select value={d.unidadMedida ?? 'NIU'}
                                    onChange={(e) => { const nuevos = [...detalles]; nuevos[i] = { ...nuevos[i], unidadMedida: e.target.value }; setDetalles(nuevos); }}
