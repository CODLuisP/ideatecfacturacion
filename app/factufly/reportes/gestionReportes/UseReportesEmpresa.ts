...
import { ReportesData, ClienteExport, ReportesEmpresaParams } from './Reportes'

interface UseReportesEmpresaReturn {
  reportes: ReportesData | null
  loading: boolean
  loadingExport: boolean
  error: string | null
  fetchReportes: (params: ReportesEmpresaParams) => Promise<ReportesData | null>
  fetchExport: (params: Omit<ReportesEmpresaParams, 'limite'>) => Promise<ClienteExport[]>
  reset: () => void
}

export const useReportesEmpresa = (): UseReportesEmpresaReturn => {
  ...
  const fetchReportes = useCallback(async ({
    ruc, periodo, desde, hasta, limite = 10, usuarioId
  }: ReportesEmpresaParams): Promise<ReportesData | null> => {
    setLoading(true)
    setError(null)
    try {
      const url = buildUrl(`/api/Reportes/empresa/${ruc}`, {
  ...
  }: Omit<ReportesEmpresaParams, 'limite'>): Promise<ClienteExport[]> => {
    setLoadingExport(true)
    try {
      const url = buildUrl(`/api/Reportes/empresa/${ruc}/export`, {
