...
import { X, CreditCard, Calendar, Clock, History, ChevronDown, RefreshCw } from 'lucide-react'
import { cn } from '@/app/utils/cn'
import { DeudaContado, PagoDeudaContado, RegistrarPagoDeudaPayload } from '@/app/factufly/deudasporcobrar/gestionDeudasPorCobrar/DeudaContado'
import { formatFecha, formatMoneda, tipoComprobanteLabel, MEDIO_PAGO_OPTS } from '@/app/factufly/cuentasporcobrar/gestionCuentasPorCobrar/helpers'
...
export const ModalDeudasPorCobrar = ({
  deuda, onClose, onConfirm, loading, usuarioId
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 flex flex-col animate-in zoom-in-95 duration-200" style={{ maxHeight: '90vh' }}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs mt-0.5 text-blue-200">
                {deuda.numeroCompleto} · {tipoComprobanteLabel(deuda.tipoComprobante)} · {formatFecha(deuda.fechaEmision)}
              </p>
            </div>
          </div>
        ...
        <div className="px-6 py-5 space-y-4 overflow-y-auto flex-1">
          ...
          <div className="bg-gray-50 rounded-xl px-4 py-3 space-y-0.5">
            <p className="text-xs font-bold text-gray-900">{deuda.clienteRznSocial}</p>
            <p className="text-xs text-gray-500">
              {deuda.clienteNumDoc.length === 11 ? 'RUC' : 'DNI'}: {deuda.clienteNumDoc}
            </p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-gray-50 rounded-xl px-3 py-2.5 text-center">
              ...
              <p className="text-sm font-bold text-gray-900">{formatMoneda(deuda.montoTotal, deuda.tipoMoneda)}</p>
            </div>
            <div className="bg-emerald-50 rounded-xl px-3 py-2.5 text-center">
              ...
              <p className="text-sm font-bold text-emerald-600">{formatMoneda(deuda.montoPagado, deuda.tipoMoneda)}</p>
            </div>
            <div className="bg-blue-50 rounded-xl px-3 py-2.5 text-center">
              ...
              <p className="text-sm font-bold text-blue-700">{formatMoneda(montoRestante, deuda.tipoMoneda)}</p>
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <input type="date" value={fechaPago} max={hoy}
          ...
          <div className="space-y-1.5">
            ...
            <input type="number" value={montoPagado} min={0.01} max={montoRestante} step="0.01"
              ...
              <div className="mt-2 space-y-1">
                ...
                <div className="flex justify-between text-[10px]">
                  <span className="text-gray-400">
                    Quedaría: <span className="font-semibold text-gray-600">{formatMoneda(quedaria, deuda.tipoMoneda)}</span>
                  </span>
              ...
              {historialAbierto && (
                <div className="animate-in slide-in-from-top-1 duration-200">
                  {hookHistorial.loading ? (
                    ...
                    <div className="py-5 text-center text-xs text-gray-400">Sin registros de pago</div>
                  ) : (
                    <table className="w-full text-xs">
                      ...
                      <tbody className="divide-y divide-gray-50">
                        {historial.map(p => (
                          <tr key={p.deudaPagoID} className="hover:bg-gray-50/50 transition-colors">
