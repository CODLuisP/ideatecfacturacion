...
import { useReporteDeudaContado } from '@/app/factufly/deudasporcobrar/gestionDeudasPorCobrar/UseReporteDeudaContado'
...
interface ModalReporteDeudaContadoProps {
  empresaRuc: string
...
export const ModalReporteDeudaContado = ({
  empresaRuc, isSuperAdmin, onClose
}: ModalReporteDeudaContadoProps) => {

  const { loading, descargarReporte } = useReporteDeudaContado()
  ...
  const tituloAuto = () => {
    const partes = [`Deudas-por-Cobrar_${empresaRuc}`]
  ...
  const limpiar = () => {
    ...
    setClienteNumDoc(''); setTituloReporte(''); setShortcutActivo('')
  }

  const filtrosActivos = [
    isSuperAdmin && sucursalFiltro && (sucursales.find(s => s.codEstablecimiento === sucursalFiltro)?.nombre ?? sucursalFiltro),
    clienteNumDoc,
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 flex flex-col animate-in zoom-in-95 duration-200">
        ...
        <div className="px-6 pt-5 pb-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-bold text-gray-900">Reporte Deudas por Cobrar</h2>
